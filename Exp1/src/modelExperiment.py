...
from Controller import HumanController, ModelController, ModelControllerWithGoal, NormalNoise, AwayFromTheGoalNoise, CheckBoundary, InferGoalPosterior, SoftmaxPolicy
import UpdateWorld
...
def main():
    ...
    Q_dict = pickle.load(open(machinePolicyPath + "noise0.1commitSTGoalGird15_policy.pkl", "rb"))

    softmaxBeta = 2.5
    goalPolicy = SoftmaxPolicy(Q_dict, softmaxBeta)

    initPrior = [0.5, 0.5]
    commitBetaList = np.arange(10, 20, 2)
    ...
    for commitBeta in commitBetaList:
        for i in range(30):
            ...
            modelControllerWithGoal = ModelControllerWithGoal(dimension, softmaxBeta, goalPolicy, Q_dict, commitBeta)
